void TXtiming() // Timing
   ...
   if (Outside == true)
    ...
    if ((minute() % 10 == 0) && (second() <= 2) && (telemetry_set == 1)) // TX WSPR standard message
   {
    ...
    else if ((minute() % 10 == 2) && (second() <= 1) && (telemetry_set == 2))    // TX WSPR telemetry message
   {
     // freq = freq + (random(-4, 5) * 2 * 1.46); // Randomize WSPR frequency
      GPS_VCC_off();
      rf_on();
      ...
      rf_off();
      GPS_VCC_on();
      telemetry_set = 3;            // Set WSPR as next in sequence
    }
    
    else if ((minute() % 10 == 4) && (second() <= 1) && (telemetry_set == 3))    // TX WSPR standard message
    {
     // freq = freq + (random(-4, 5) * 2 * 1.46); // Randomize WSPR frequency
      GPS_VCC_off(); 
      rf_on();
      ...
      rf_off(); 
      GPS_VCC_on();
      telemetry_set = 4;             // Set WSPR as next in sequence
   }
   
    else if ((minute() % 10 == 6) && (second() <= 1) && (telemetry_set == 4))    // TX WSPR standard message
   {
     // freq = freq + (random(-4, 5) * 2 * 1.46); // Randomize WSPR frequency
      GPS_VCC_off();
      rf_on();  
